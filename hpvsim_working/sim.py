...
from . import network as hpn #NEW!! 
...
class Sim(hpb.BaseSim):
    ...
    def __init__(self, pars=None, datafile=None, label=None,
        ...
        self.network_backend = None   # NEW!! for storing base network structures
        self.network_delta = None     # NEW!! storing changes in networks
    ...
    #NEW!! added network init
    def initialize(self, reset=True, init_states=True, init_intvs=True, init_analyzers=True, init_network_backend=True, **kwargs):
        '''
        ...
        if init_network_backend: self.init_network_backend() #NEW!! sim.step() always routes partnership formation/dissolution through the backend, so this must run (needs data from an already-made population, hence after init_people)
    ...
    def community_labels(self):
        '''
        Names of the communities used to label the by-community results, taken from
        pars['community_pars']['community_labels'] if supplied (e.g. a list of ethnicities) and
        otherwise generated as 'Community 0', 'Community 1', ...
        '''
        nc = int(self['community_pars']['n_communities'])
        labels = self['community_pars'].get('community_labels')
        if labels is None:
            return [f'Community {i}' for i in range(nc)]
        labels = [str(label) for label in labels]
        if len(labels) != nc:
            errormsg = (f"community_pars['community_labels'] has {len(labels)} entries but "
                        f"n_communities is {nc}; they must match.")
            raise ValueError(errormsg)
        return labels


    #NEW!!
    def community_result_stems(self):
        '''
        Which results are stratified by community in this sim, as a list of stems (the result keys
        are these plus '_by_community'). Driven by pars['community_results'], which accepts True
        for everything, False for nothing, one of the preset names in
        defaults.community_result_presets ('epi', 'rates', 'flows', 'stocks', 'network',
        'minimal', 'all'), or an explicit list of result names. A selection is automatically
        widened to include anything its members need in order to be computed.
        '''
        return hpd.resolve_community_results(self['community_results'], self.layer_keys())
    ...
    def validate_pars(self, validate_layers=True):
        ...
        #NEW!! Community-stratified results only have a source of community membership when the
        # community network backend is in use -- it is the only thing that assigns communities
        # (see community_network.CommunityNetworkBackend). With any other network every person
        # would stay at community=-1 and every by-community result would be identically zero.
        if self['community_results'] and choice != 'community':
            errormsg = (f"community_results=True requires network='community', but network="
                        f"'{choice}' was requested. Communities are assigned by "
                        f"CommunityNetworkBackend at network entry; no other network defines them.")
            raise ValueError(errormsg)
    ...
    def init_results(self, frequency='annual', add_data=True):
        ...
        #NEW!! Results stratified by community. Only created when community_results is on, so with
        # the toggle off sim.results is exactly what it was before this feature existed.
        # init_results() runs before the people and the network backend exist (see initialize()),
        # so the number of communities has to come from the parameters rather than from the backend.
        if self['community_results']:
            nc = int(self['community_pars']['n_communities'])
            clabels = self.community_labels()  # Called here so a bad community_labels fails at init, not at plot/export time
            stems = self.community_result_stems()

            # Everything that *can* be stratified by community, as stem -> (label, extra kwargs).
            # Only the stems selected by pars['community_results'] are actually created -- the
            # selection has already been expanded to include whatever compute_states() needs to
            # fill them (see defaults.resolve_community_results), so a partial selection can never
            # leave a result that cannot be computed.
            opts = {}

            # Flows by community, mirroring the by-age flows above. These sum back to the
            # population-wide flow except for events happening to people who have no community
            # yet: infections seeded at t=0 land on anyone above init_hpv_prev's first age
            # bracket, including people who have not yet had their sexual debut, and the CINs and
            # cancers those seeds go on to cause before the person debuts are counted in the
            # population-wide flow but have no community to be counted in
            for flow in hpd.flows:
                opts[flow.name] = (flow.label+' by community', dict(color=flow.color))

            # Stocks by community, for the prevalence/incidence calculations in compute_states()
            opts['n_infectious']  = ('Number infectious by community', dict(color=stock_colors[0]))
            opts['n_susceptible'] = ('Number susceptible by community', {})
            opts['n_precin']      = ('Number Pre-CIN by community', {})
            opts['n_cin']         = ('Number CIN by community', {})
            opts['n_cancerous']   = ('Number with cancer by community', {})
            opts['n_vaccinated']  = ('Number vaccinated by community', {})

            # Denominators. NB, these count network members only: people who have not yet had their
            # sexual debut have no community (people.community==-1) and are excluded throughout,
            # so these do not sum to n_alive
            opts['n_alive']         = ('Number alive by community', {})
            opts['n_females_alive'] = ('Number females alive by community', {})

            # Network structure by community
            opts['n_partnerships'] = ('Number of partnerships by community', {})
            opts['mean_degree']    = ('Mean degree by community', dict(scale=False))
            for lkey in self.layer_keys():
                opts[f'n_partnerships_{lkey}'] = (f'Number of {lkey}-layer partnerships by community', {})
                opts[f'mean_degree_{lkey}']    = (f'Mean {lkey}-layer degree by community', dict(scale=False))
            opts['within_edge_frac'] = ('Fraction of partnerships within own community', dict(scale=False))
            opts['single_frac']      = ('Fraction with no current partners by community', dict(scale=False))

            # Prevalence and incidence by community, computed in compute_states()
            opts['hpv_prevalence']    = ('HPV prevalence by community', dict(scale=False, color=stock_colors[0]))
            opts['hpv_incidence']     = ('HPV incidence by community', dict(scale=False))
            opts['precin_prevalence'] = ('Pre-CIN prevalence by community', dict(scale=False, color=stock_colors[0]))
            opts['cin_prevalence']    = ('CIN prevalence by community', dict(scale=False, color=stock_colors[1]))
            opts['cancer_incidence']  = ('Cancer incidence by community', dict(scale=False))
            opts['cancer_mortality']  = ('Cancer mortality by community', dict(scale=False))

            for stem in stems:
                label, kwargs = opts[stem]
                results[stem+'_by_community'] = init_res(label, n_rows=nc, **kwargs)

            if self['verbose'] > 0:
                print(f'  storing {len(stems)} results by community ({sc.strjoin(clabels)})')
    ...
    #NEW!!
    def init_network_backend(self):
        """
        Initialize and register the network backend that drives partnership
        formation/dissolution each timestep (see network.py). Unlike analyzers, the
        backend is required infrastructure -- sim.step() always calls it -- so it is
        stored directly on the sim as ``self.network_backend`` rather than being
        added to ``self['analyzers']``. To record the per-timestep deltas it produces,
        attach the ``network_history`` analyzer instead.
        """
        if self.network_backend is None:
            self.network_backend = hpn.make_network_backend(self)

        # Initialize here (rather than in init_analyzers) so the backend is ready
        # even if init_analyzers=False, and so analyzers that depend on it (e.g.
        # network_history) can read sim.network_backend during their own initialize().
        self.network_backend.initialize(self)
    ...
    def step(self):
        ...
        #NEW!! Reset the network backend's per-timestep bookkeeping before *anything* that can
        # add/remove nodes or edges this timestep runs (including the HIV substep below).
        self.network_backend.begin_step(self)
        ...
        #NEW!! this now sits within the backend's step function (ages people, applies deaths,
        # generates new births, dissolves expired/orphaned partnerships, and creates new ones):
        self.network_backend.step(self)
        ...
        #NEW!! Per-timestep who-infected-whom buffer, in the same idiom as sim.network_delta:
        # written here, read by analyzers at the bottom of step(), cleared immediately after.
        track_tx = bool(self['track_transmission'])
        people.new_transmissions = []
        ...
        for lkey, layer in people.contacts.items():
            ...
            for g in range(ng):
                ...
                for pship_inds, sources, targets, this_foi in discordant_pairs:
                    ...
                    #NEW!! Record the infector for each infection. `sources` is aligned with
                    # `targets` edge-for-edge, so sources[transmissions] are the infectors of
                    # targets[transmissions]; np.unique's return_index gives the position within
                    # that array of the retained (first) occurrence of each target, so indexing by
                    # unique_inds keeps the pairing exact. Must be taken BEFORE people.infect(),
                    # which can grow the population and invalidate nothing here but is clearer
                    # ordered this way.
                    if track_tx:
                        # people.scale must be read HERE, not later: for ms_agent_ratio > 1,
                        # people.infect() -> set_severity() shrinks the scale of every agent that
                        # transforms, while infect()'s own scale_flows() call reads it beforehand.
                        # Sampling it after the fact would undercount those agents' infections and
                        # break agreement with results['infections'].
                        people.new_transmissions.append((
                            sources[transmissions][unique_inds].copy(),
                            target_inds.copy(),
                            people.scale[target_inds].copy(),
                            g,
                            lkey,
                        ))
        ...
        #NEW!! Build this timestep's network delta now, after everything that can add/remove
        # nodes or edges has run (demographics, interventions, and -- notably -- the transmission
        # loop above, which can grow multiscale cancer-agent clones via people.infect()).
        self.network_delta = self.network_backend.finalize_step(self)
        ...
        #NEW!! Flows by community. people.community_flows is empty unless community_results is on,
        # which makes this loop a no-op rather than something that needs its own conditional
        for key,count in people.community_flows.items():
            reskey = key+'_by_community'
            if reskey in self.results: # Not every flow is necessarily selected
                self.results[reskey][:,idx] += count
        ...
        if t % self.resfreq == self.resfreq-1:
            ...
            #NEW!! Stocks and network structure by community
            if self['community_results']:
                self.update_community_results(idx)

        # Apply analyzers
        for i,analyzer in enumerate(self.analyzers):
            analyzer(self)
        
        self.network_delta = None     #NEW!! reset network delta here for next time step, the history analyser would have already copied in what is needed
        people.new_transmissions = []  #NEW!! ditto for the transmission buffer and the
        people.ms_clone_log = []       # multiscale-clone parentage log (see analysis.core_group_attribution)
    ...
    #NEW!!
    def update_community_results(self, idx):
        '''
        Fill in the by-community stock and network-structure results for this result timepoint.
        Called from step() on the same schedule as the by-age stocks, and only when
        community_results is on.

        Everything here is restricted to alive network members -- people who have not yet had
        their sexual debut have never entered the network and so have people.community == -1
        (see community_network.CommunityNetworkBackend) -- and is weighted by people.scale, in
        keeping with the rest of the results.

        Only the results that pars['community_results'] actually selected are stored, and the
        work behind a whole group is skipped when none of that group was asked for.
        '''
        people = self.people
        nc = people.nc
        comm = people.community
        res = self.results

        def wanted(*stems):
            ''' Whether any of these results is being stored '''
            return any(f'{stem}_by_community' in res for stem in stems)

        def store(stem, values):
            ''' Store a by-community result if it was selected; always return it for reuse '''
            key = f'{stem}_by_community'
            if key in res:
                res[key][:, idx] = values
            return values

        # Stocks. The disease states are (n_genotypes, n_people), and there are two ways to count
        # them: once per infected *person* (as the population-wide n_infectious does, via
        # people.count_any) or once per person-*genotype* pair (as n_infectious_by_age does, via
        # hpu.true, which returns one entry per genotype a person is infected with). The
        # by-community stocks deliberately use the per-person convention wherever they feed a
        # prevalence, so that e.g. hpv_prevalence_by_community is a true proportion in [0, 1] and
        # is directly comparable to hpv_prevalence -- the by-age results are not, since
        # co-infections there are counted once per genotype.
        # n_susceptible_by_community is the exception: it is a denominator for incidence, whose
        # numerator (infections_by_community) counts infection events, so it counts person-genotype
        # pairs, exactly as n_susceptible_by_age does
        def people_with(key):
            ''' Person indices with `key` true for at least one genotype '''
            return people[key].any(axis=0).nonzero()[0]

        # n_alive is kept as a local as well as a result, since the network results below divide
        # by it whether or not it was itself selected for storage
        n_members = people.bin_communities(hpu.true(people.alive))
        store('n_alive', n_members)
        if wanted('n_females_alive'):
            store('n_females_alive', people.bin_communities(hpu.true(people.alive*people.is_female)))
        if wanted('n_infectious'):
            store('n_infectious', people.bin_communities(people_with('infectious')))
        if wanted('n_susceptible'):
            store('n_susceptible', people.bin_communities(hpu.true(people['susceptible'])))
        if wanted('n_precin'):
            store('n_precin', people.bin_communities(people_with('precin')))
        if wanted('n_cin'):
            store('n_cin', people.bin_communities(people_with('cin')))
        if wanted('n_cancerous'):
            store('n_cancerous', people.bin_communities(people_with('cancerous')))
        if wanted('n_vaccinated'):
            store('n_vaccinated', people.bin_communities(hpu.true(people['vaccinated'])))

        # Network structure. Degree comes from people.current_partners, which the network backend
        # maintains per layer, so the per-layer split is free. Each result counts partnership
        # *ends* held by members of the community: a partnership between two members of the same
        # community contributes 1 to that community for each of its two ends
        layer_stems = [f'{prefix}_{lkey}' for lkey in self.layer_keys()
                       for prefix in ('n_partnerships', 'mean_degree')]
        if wanted('n_partnerships', 'mean_degree', 'single_frac', *layer_stems):
            members = (people.alive & (comm >= 0)).nonzero()[0]
            member_comm = comm[members]
            member_scale = people.scale[members]
            current = people.current_partners
            total_degree = np.zeros(nc, dtype=hpd.result_float)
            for ln, lkey in enumerate(self.layer_keys()):
                degree = np.bincount(member_comm, weights=current[ln, members]*member_scale, minlength=nc)
                store(f'n_partnerships_{lkey}', degree)
                store(f'mean_degree_{lkey}', sc.safedivide(degree, n_members))
                total_degree += degree
            store('n_partnerships', total_degree)
            store('mean_degree', sc.safedivide(total_degree, n_members))

            # People with no current partners in any layer
            if wanted('single_frac'):
                single_inds = members[current[:, members].sum(axis=0) == 0]
                store('single_frac', sc.safedivide(people.bin_communities(single_inds), n_members))

        # Assortativity: what share of each community's partnership ends are with a partner from
        # the same community. This one needs the actual endpoint pairs, so it reads the contacts
        if wanted('within_edge_frac'):
            within = np.zeros(nc, dtype=hpd.result_float)
            total = np.zeros(nc, dtype=hpd.result_float)
            for lkey, layer in people.contacts.items():
                f, m = layer['f'], layer['m']
                if not len(f):
                    continue
                cf, cm = comm[f], comm[m]
                valid = (cf >= 0) & (cm >= 0)
                cf, cm = cf[valid], cm[valid]
                wf, wm = people.scale[f[valid]], people.scale[m[valid]]
                total += np.bincount(cf, weights=wf, minlength=nc) + np.bincount(cm, weights=wm, minlength=nc)
                same = cf == cm
                within += (np.bincount(cf[same], weights=wf[same], minlength=nc)
                           + np.bincount(cm[same], weights=wm[same], minlength=nc))
            store('within_edge_frac', sc.safedivide(within, total))
    ...
    def finalize(self, verbose=None, restore_pars=True):
        ...
        if self.network_backend is not None:
            self.network_backend.finalize(self)
    ...
    def compute_states(self):
        ...
        #NEW!! The same rates by community. Denominators count network members only (people who
        # have not yet debuted have no community), so these are rates *within* each community and
        # do not average back to the population-wide rates above
        # Each rate is only computed if it was selected; resolve_community_results() guarantees
        # that whatever a selected rate divides is present too
        if self['community_results']:
            def rate(stem, get):
                if f'{stem}_by_community' in res:
                    self.results[f'{stem}_by_community'][:] = get()
                return
            comm = lambda stem: res[f'{stem}_by_community'][:]
            rate('hpv_prevalence',    lambda: safedivide(comm('n_infectious'), comm('n_alive')))
            rate('hpv_incidence',     lambda: safedivide(comm('infections'), comm('n_susceptible')))
            rate('precin_prevalence', lambda: safedivide(comm('n_precin'), comm('n_females_alive')))
            rate('cin_prevalence',    lambda: safedivide(comm('n_cin'), comm('n_females_alive')))
            rate('cancer_incidence',  lambda: safedivide(comm('cancers'),
                                                         (comm('n_females_alive') - comm('n_cancerous'))/scale_factor))
            rate('cancer_mortality',  lambda: safedivide(comm('cancer_deaths'),
                                                         comm('n_females_alive')/scale_factor))
