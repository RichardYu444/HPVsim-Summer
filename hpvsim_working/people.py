...
class People(hpb.BasePeople):
    ...
    **Examples**::
    ...
    def __init__(self, pars, strict=True, pop_trend=None, pop_age_trend=None, **kwargs):
        ...
        #NEW!! Per-timestep bookkeeping consumed by the network backend to build a NetworkDelta
        # without having to diff the whole population/contacts each step. Cleared each timestep
        # by NetworkBackend.begin_step(); harmless (just grows a little) if no backend is attached.
        self.network_added_node_log = []      # list of (inds, entry_kind), appended by add_births
        self.network_removed_node_log = []    # list of (inds, cause), appended by remove_people
        self.network_added_edges = {}         # lkey -> dict of eid/f/m/... , set by create_partnerships
        self.network_dissolved_edges = {}     # lkey -> dict of eid/f/m/reason, set by dissolve_partnerships
        #NEW!! Multiscale cancer-agent parentage, appended by set_severity() and drained each
        # timestep by core_group_attribution (analysis.py). Kept as its OWN list rather than
        # extending network_added_node_log's tuples, which network.py consumes as 2-tuples.
        self.ms_clone_log = []                # list of (new_inds, parent_inds)
        self.new_transmissions = []           # list of (source_inds, target_inds, genotype, lkey), set by Sim.step when pars['track_transmission']
        self.ng = self.pars["n_genotypes"]
        self.na = len(self.pars["age_bin_edges"]) - 1
        #NEW!! Number of communities to stratify results by; 0 when community_results is off, which
        # is what makes every community code path below a no-op (see init_flows)
        self.nc = (
            int(self.pars["community_pars"]["n_communities"])
            if self.pars.get("community_results")
            else 0
        )
        ...
        for lkey in self.layer_keys():
            self.rship_lags[lkey] = np.zeros(
                len(self.lag_bins) - 1, dtype=hpd.default_float
            )
    ...
    def init_flows(self):
        ...
        self.genotype_flows = {
            key: np.zeros(self.ng, dtype=df) for key in hpd.genotype_flow_keys
        }
        ...
        #NEW!! Left empty unless community_results is on, which makes the consumer loop in
        # Sim.step() a no-op rather than something that has to be conditional
        self.community_flows = (
            {key: np.zeros(self.nc, dtype=df) for key in hpd.flow_keys} if self.nc else {}
        )
        return
    ...
    def bin_communities(self, inds):
        """
        NEW!! Scale-weighted counts of inds by community -- the by-community counterpart of the
        np.histogram(self.age[inds], bins=self.age_bin_edges, weights=self.scale[inds]) idiom used
        for the by-age results. People with community<0 (never entered the network, i.e. pre-debut)
        are dropped: they have no community, so they contribute to neither the numerators nor the
        denominators of any by-community result.
        """
        comm = self.community[inds]
        valid = comm >= 0
        return np.bincount(
            comm[valid], weights=self.scale[inds][valid], minlength=self.nc
        )
    ...
    def initialize(self, sim_pars=None):
        ...
        if "current_partners" in kwargs:
            self.current_partners[:] = kwargs.pop(
                "current_partners"
            )  # Store current actual number - updated each step though
        ...
        self.validate(
            sim_pars=sim_pars
        )  # First, check that essential-to-match parameters match
    ...
    def update_states_pre(self, t, year=None):
        ...
        update_freq = max(
            1, int(self.pars["dt_demog"] / self.pars["dt"])
        ...
        self.sus_pool = self.susceptible.all(
            axis=0
        )  # True for people with no infection at the start of the timestep
    ...
    def set_prognoses(self, inds, g, gpars, dt):
        ...
        dur_precin = hpu.sample(**gpars["dur_precin"], size=len(inds)) * (
            1 - sev_imm
        ...
        cin_probs = hppar.compute_severity(
            dur_precin, rel_sev=self.rel_sev[inds], pars=gpars["cin_fn"]
        )
        ...
        age_mod[self.age[cin_inds] >= self.pars["age_risk"]["age"]] = self.pars[
            "age_risk"
        ]["risk"]
        ...
        self.date_clearance[g, nocin_inds] = self.t + sc.randround(
            self.dur_precin[g, nocin_inds] / dt
        )

        # Set date of onset of precancer for those who develop precancer
        self.date_cin[g, cin_inds] = self.t + sc.randround(
            self.dur_precin[g, cin_inds] / dt
        )
    ...
    def set_severity(self, inds, g, gpars, dt, set_sev=True):
        ...
        cancer_prob = hppar.compute_severity(
            dur_cin, rel_sev=self.rel_sev[inds], pars=cancer_pars
        )  # Calculate probability of cancer
        n_extra = self.pars["ms_agent_ratio"]
        cancer_scale = self.pars["pop_scale"] / n_extra

        if n_extra == 1:
            cancer_prob_arr = hppar.compute_severity(
                dur_cin, rel_sev=self.rel_sev[inds], pars=cancer_pars
            )
        # Multiscale version
        elif n_extra > 1:
            ...
            is_cancer = hpu.binomial_arr(
                cancer_prob
            )  # Select who transforms - NB, this array gets extended later
            cancer_inds = inds[is_cancer]  # Indices of those who transform
            self.scale[cancer_inds] = (
                cancer_scale  # Shrink the weight of the original agents, but otherwise leave them the same
            )
            ...
            extra_cancer_bools *= self.level0[
                inds, None
            ]  # Don't allow existing cancer agents to make more cancer agents
            extra_cancer_counts = extra_cancer_bools.sum(
                axis=1
            )  # Find out how many new cancer cases we have
            ...
            if n_new_agents:  # If we have more than 0, proceed
                ...
                for i, count in enumerate(extra_cancer_counts):
                    ...
                    if (
                        count
                    ):  # At least 1 new cancer agent, plus person is not already a cancer agent
                        extra_source_lists.append(
                            [ii] * int(count)
                        )  # Duplicate the current index count times
                extra_source_inds = np.concatenate(
                    extra_source_lists
                ).flatten()  # Assemble the sources for these new agents
                n_new_agents = len(
                    extra_source_inds
                )  # The same as above, *unless* a cancer agent tried to spawn more cancer agents
                ...
                for state in self.meta.states_to_set:
                    if state.ndim == 1:
                        ...
                        self[state.name][:, new_inds] = self[state.name][
                            :, extra_source_inds
                        ]
                ...
                #NEW!! these agents are grown directly (not via add_births), so log them here too
                self.network_added_node_log.append((new_inds.copy(), hpnet.ENTRY_MULTISCALE))
                #NEW!! ...and record which level-0 agent each clone was cloned FROM. A clone has
                # no infection event of its own -- its causal infection is its parent's -- so any
                # cancer it goes on to develop must be attributed through extra_source_inds.
                self.ms_clone_log.append((new_inds.copy(), extra_source_inds.copy()))

                # Add the new indices onto the existing vectors
                inds = np.append(inds, new_inds)
                is_cancer = np.append(
                    is_cancer, np.full(len(new_inds), fill_value=True)
                )
            ...
            cancer_prob_arr[is_cancer] = (
                1  # Make sure inds that got assigned cancer above dont get stochastically missed
            )
        ...
        no_cancer_inds = inds[
            ~is_cancer
        ]  # Indices of those who eventually heal lesion/clear infection

        # Set date of clearance for those who don't go to cancer
        dur_inf = self.dur_infection[g, inds]
        time_to_clear = dur_inf[~is_cancer]
        self.date_clearance[g, no_cancer_inds] = np.fmax(
            self.date_clearance[g, no_cancer_inds],
            self.date_exposed[g, no_cancer_inds] + sc.randround(time_to_clear / dt),
        )
        ...
        dur_cin_transformed = dur_cin[
            is_cancer
        ]  # Duration of episomal infection for those who transform
        self.date_cancerous[g, cancer_inds] = self.date_cin[
            g, cancer_inds
        ] + sc.randround(dur_cin_transformed / dt)
        self.dur_infection[g, cancer_inds] = (
            self.dur_precin[g, cancer_inds] + self.dur_cin[g, cancer_inds]
        )
        dur_cancer = hpu.sample(**self.pars["dur_cancer"], size=len(cancer_inds))
        self.date_dead_cancer[cancer_inds] = self.date_cancerous[
            g, cancer_inds
        ] + sc.randround(dur_cancer / dt)
    ...
    def dissolve_partnerships(self, t=None):
        ...
        n_dissolved = dict()
        dissolved_edges = dict()  #NEW!! captured for the network backend's delta this timestep

        for lno, lkey in enumerate(self.layer_keys()):
            layer = self.contacts[lkey]

            node_removed = (~self["alive"][layer["m"]]) + (~self["alive"][layer["f"]])  #NEW!! split out so we can record *why* each edge dissolved
            to_dissolve = (
                node_removed
                + ((self.t * self.pars["dt"]) > layer["end"]).astype(bool)
            )
            dissolved = layer.pop_inds(
                to_dissolve
            )  # Remove them from the contacts list

            # Update current number of partners
            unique, counts = hpu.unique(
                np.concatenate([dissolved["f"], dissolved["m"]])
            )
            self.current_partners[lno, unique] -= counts
            self.rship_end_dates[lno, unique] = self.t
            n_dissolved[lkey] = len(dissolved["f"])

            #NEW!! record the dissolved edges (with a reason) for the network backend to pick up
            if len(dissolved["f"]):
                reason = np.where(
                    node_removed[to_dissolve], hpnet.EDGE_NODE_REMOVED, hpnet.EDGE_EXPIRED
                ).astype(np.int8)
                dissolved_edges[lkey] = dict(
                    eid=dissolved["eid"], f=dissolved["f"], m=dissolved["m"], reason=reason
                )

        self.network_dissolved_edges = dissolved_edges  #NEW!!
    ...
    def create_partnerships(
        ...
        for lkey in self.layer_keys():
            pship_args = dict(
                lno=lno,
                tind=tind,
                partners=self.partners[lno],
                current_partners=self.current_partners,
                ages=self.age,
                debuts=self.debut,
                ...
                add_mixing=self.pars["add_mixing"],
            )
            new_pships[lkey], current_partners, new_pship_inds, new_pship_counts = (
                hppop.make_contacts(**pship_args)
            )
            ...
            if len(new_pship_inds):
                ...
                lags = (
                    self.rship_start_dates[lno, new_pship_inds]
                    - self.rship_end_dates[lno, new_pship_inds]
                )
                self.rship_lags[lkey] += np.histogram(lags, self.lag_bins)[0]

            lno += 1

        added = self.add_contacts(new_pships)  #NEW!! capture the eid-enriched contacts that were actually added
        self.network_added_edges = added       #NEW!! for the network backend's delta this timestep
    ...
    def check_progress(self, what, genotype):
        ...
        if what == "cins":
            cases_by_age, cases = self.check_cin(genotype)
        elif what == "cancers":
            cases_by_age, cases = self.check_cancer(genotype)
    ...
    def check_cin(self, genotype):
        ...
        filters = (
            self.infectious[genotype, :]
            * self.is_female_alive
            * ~(self.date_clearance[genotype, :] <= self.t)
        )
        filter_inds = filters.nonzero()[0]
        inds = self.check_inds(
            self.cin[genotype, :], self.date_cin[genotype, :], filter_inds=filter_inds
        )
        self.cin[genotype, inds] = True

        # Age calculations
        cases_by_age = np.histogram(
            self.age[inds], bins=self.age_bin_edges, weights=self.scale[inds]
        )[0]
        #NEW!! Community calculations. Accumulated in place (rather than returned alongside
        # cases_by_age) so no call signature changes; summing over genotypes here matches what the
        # caller does with cases_by_age in update_states_post()
        if self.nc:
            self.community_flows["cins"] += self.bin_communities(inds)
        return cases_by_age, self.scale_flows(inds)

    def check_cancer(self, genotype):
        """Check for new progressions to cancer"""
        not_current = hpu.ifalsei(
            self.cancerous[genotype, :], hpu.true(self.cin[genotype, :])
        )
        ...
        self.infectious[:, inds] = (
            False  # No longer counted as infectious with any genotype
        )
        self.inactive[:, inds] = (
            True  # If this person has any other infections from any other genotypes, set them to inactive
        )
        self.date_clearance[:, inds] = (
            np.nan
        )  # Remove their clearance dates for all genotypes

        # Deal with dysplasia states and dates
        for g in range(self.ng):
            if g != genotype:
                self.date_cancerous[g, inds] = (
                    np.nan
                )  # Remove their date of cancer for all genotypes but the one currently causing cancer
        ...
        #NEW!! Community results (see check_cin)
        if self.nc:
            self.community_flows["cancers"] += self.bin_communities(inds)
        ...
        if self.pars["model_hiv"]:
            ...
            self.hivsim.results["cancers_with_hiv"][idx] += self.scale_flows(
                hiv_pos_inds
            )
            self.hivsim.results["cancers_no_hiv"][idx] += self.scale_flows(hiv_neg_inds)
            self.hivsim.results["cancers_by_age_with_hiv"][:, idx] += np.histogram(
                self.age[hiv_pos_inds],
                bins=self.age_bin_edges,
                weights=self.scale[hiv_pos_inds],
            )[0]
            self.hivsim.results["cancers_by_age_no_hiv"][:, idx] += np.histogram(
                self.age[hiv_neg_inds],
                bins=self.age_bin_edges,
                weights=self.scale[hiv_neg_inds],
            )[0]
    ...
    def check_cancer_deaths(self):
        ...
        inds = self.check_inds(
            self.dead_cancer, self.date_dead_cancer, filter_inds=filter_inds
        )
        ...
        #NEW!! Community results (see check_cin). Communities are not cleared on death, so the
        # people just removed above are still binned into the community they died in
        if self.nc:
            self.community_flows["cancer_deaths"] += self.bin_communities(inds)
    ...
    def check_clearance(self, genotype):
        ...
        """
        f_filter_inds = (self.is_female_alive & self.infectious[genotype, :]).nonzero()[
            -1
        ]
        m_filter_inds = (self.is_male_alive & self.infectious[genotype, :]).nonzero()[
            -1
        ]
        f_inds = self.check_inds_true(
            self.infectious[genotype, :],
            self.date_clearance[genotype, :],
            filter_inds=f_filter_inds,
        )
        m_inds = self.check_inds_true(
            self.infectious[genotype, :],
            self.date_clearance[genotype, :],
        ...
        if self.pars["hpv_control_prob"] > 0:
            latent_probs = np.full(
                len(f_inds), self.pars["hpv_control_prob"], dtype=hpd.default_float
            )
            latent_bools = hpu.binomial_arr(latent_probs)
            latent_inds = f_inds[latent_bools]

            if len(latent_inds):
                self.susceptible[genotype, latent_inds] = (
                    False  # should already be false
                )
        ...
        if len(f_cleared_inds):
            hpimm.update_peak_immunity(
                self, f_cleared_inds, imm_pars=self.pars, imm_source=genotype
            )  # update immunity
    ...
    def add_births(
        ...
        sex_ratio=0.5,
        hiv_prevalence=None,
        entry_kind=None,  #NEW!! lets callers (e.g. migration) tag these new agents distinctly from ordinary births
    ):
        """
        ...
        if new_births is None:
            ...
            new_births = sc.randround(
                this_birth_rate * self.n_alive_level0
            )  # Crude births per 1000

        if new_births > 0:
            # Generate other characteristics of the new people
            uids, sexes, debuts, rel_sev, partners, cluster = hppop.set_static(
                new_n=new_births,
                existing_n=len(self),
                pars=self.pars,
                sex_ratio=sex_ratio,
            ...
            if hiv_prevalence is not None and len(hiv_prevalence):
                ...
                new_hiv = new_inds[hiv_bools]
                self.hivsim.set_hiv_prognoses(self, new_hiv)

            #NEW!! record the new nodes for the network backend's delta this timestep
            kind = entry_kind if entry_kind is not None else hpnet.ENTRY_BIRTH
            self.network_added_node_log.append((new_inds.copy(), kind))
    ...
    def check_migration(self, year=None):
        ...
        if self.pars["use_migration"] and self.pop_trend is not None:
            ...
            for sex in ["male", "female"]:
                ...
                alive_ages = self.age[alive_inds].astype(
                    int
                )  # Return ages for everyone level 0 and alive
                count_ages = np.bincount(
                    alive_ages, minlength=age_dist_data.shape[0]
                )  # Bin and count them
                expected = (
                    age_dist_data[sex].values * scale
                )  # Compute how many of each age we would expect in population
                difference = (expected - count_ages).astype(
                    int
                )  # Compute difference between expected and simulated for each age
                n_migrate += np.sum(difference)  # Compute total migrations (in and out)
                ages_to_remove = hpu.true(
                    difference < 0
                )  # Ages where we have too many, need to apply emigration
                n_to_remove = difference[
                    ages_to_remove
                ]  # Determine number of agents to remove for each age
                ages_to_add = hpu.true(
                    difference > 0
                )  # Ages where we have too few, need to apply imigration
                n_to_add = difference[
                    ages_to_add
                ]  # Determine number of agents to add for each age
                ...
                if len(ages_to_add):
                    ...
                    if self.pars["model_hiv"]:
                        ...
                        hiv_prevalence = safedivide(
                            np.histogram(
                                self.age[hivinds],
                                bins=np.arange(0, max(ages_to_add) + 1),
                                weights=self.scale[hivinds],
                            )[0],
                            np.histogram(
                                self.age[aliveinds],
                                bins=np.arange(0, max(ages_to_add) + 1),
                                weights=self.scale[aliveinds],
                            )[0],
                        )
                        self.add_births(
                            new_births=len(ages_to_add_list),
                            ages=np.array(ages_to_add_list),
                            sex_ratio=sex_ratio,
                            hiv_prevalence=hiv_prevalence,
                            entry_kind=hpnet.ENTRY_MIGRATION,  #NEW!! distinguish immigration from ordinary births in the network delta
                        )
    ...
    def infect(self, inds, g=None, layer=None):
        ...
        # Count reinfections and remove any previous dates. NB, .nonzero() returns positions
        # within inds, not person indices, so they have to be mapped back through inds before
        # scale_flows() (which indexes self.scale by person) -- without that, each reinfection
        # is weighted by an unrelated person's scale factor
        reinf_inds = inds[(~np.isnan(self.date_clearance[g, inds])).nonzero()[-1]]
        self.genotype_flows["reinfections"][g] += self.scale_flows(reinf_inds)
        self.flows["reinfections"] += self.scale_flows(reinf_inds)
        #NEW!! community flows (see check_cin)
        if self.nc:
            self.community_flows["reinfections"] += self.bin_communities(reinf_inds)
        ...
        if layer == "reactivation":
            ...
            self.age_flows["reactivations"] += np.histogram(
                self.age[inds], bins=self.age_bin_edges, weights=self.scale[inds]
            )[0]
            if self.nc:  #NEW!! community flows (see check_cin)
                self.community_flows["reactivations"] += self.bin_communities(inds)
        ...
        if layer != "seed_infection" and layer != "reactivation":
            ...
            self.flows["infections"] += self.scale_flows(
                inds
            )  # Add the total count to the total flow data
            self.genotype_flows["infections"][g] += self.scale_flows(
                inds
            )  # Add the count by genotype to the flow data
            self.age_flows["infections"][:] += np.histogram(
                self.age[inds], bins=self.age_bin_edges, weights=self.scale[inds]
            )[0]
            #NEW!! Community flows (see check_cin). Inside this guard, so -- exactly as for the
            # total and by-age flows -- seeded infections are excluded. Every infection counted
            # here was transmitted over a network edge, so both parties are network members and
            # this sums back to results['infections'] exactly
            if self.nc:
                self.community_flows["infections"] += self.bin_communities(inds)

            # Create by-sex flows. NB, hpu.true() returns positions within inds, not person
            # indices, so they must be mapped back through inds before scale_flows() (which
            # indexes self.scale by person) -- without that, each infection is weighted by an
            # unrelated person's scale factor
            infs_female = self.scale_flows(inds[hpu.true(self.is_female[inds])])
            infs_male = self.scale_flows(inds[hpu.true(self.is_male[inds])])
        ...
        if len(m_inds) > 0:
            dur_infection = hpu.sample(
                **self.pars["dur_infection_male"], size=len(m_inds)
            )
            self.dur_infection[g, m_inds] = dur_infection
            self.date_clearance[g, m_inds] = self.date_infectious[g, m_inds] + np.ceil(
                dur_infection / dt
            )  # Date they clear HPV infection (interpreted as the timestep on which they recover)
    ...
    def remove_people(self, inds, cause=None):
        ...
        for future_date in future_dates:
            ...
            if ndims == 1:
                ...
                genotypes_to_clear, iinds = (
                    self[future_date][:, inds] >= self.t
                ).nonzero()
                if len(iinds):
                    self[future_date][genotypes_to_clear, inds[iinds]] = np.nan

        #NEW!! record the removed nodes for the network backend's delta this timestep
        if len(inds):
            self.network_removed_node_log.append(
                (np.asarray(inds, dtype=hpd.default_int).copy(), cause)
            )
