...
class PeopleMeta(sc.prettyobj):
    ...
    def __init__(self):
        ...
        self.person = [
            ...
            State('community',      default_int,    -1),    #NEW!! Community (e.g. ethnicity) of this person, mirrored from the community network backend at network entry; -1 = not (yet) a network member
...
#%% NEW!! Results by community

# Which results can be stratified by community. Names here are the *stems*: the actual result keys
# are the stem plus '_by_community'. Split into groups so that pars['community_results'] and
# Sim.to_df(by_community=...) can be given a group name instead of a list.
community_epi_flow_keys  = [flow.name for flow in flows]
community_epi_stock_keys = ['n_infectious', 'n_susceptible', 'n_precin', 'n_cin', 'n_cancerous',
                            'n_vaccinated']
community_denom_keys     = ['n_alive', 'n_females_alive']
community_epi_rate_keys  = ['hpv_prevalence', 'hpv_incidence', 'precin_prevalence',
                            'cin_prevalence', 'cancer_incidence', 'cancer_mortality']
# The per-layer variants ('mean_degree_s', 'n_partnerships_l', ...) are added by
# community_result_stems() below, since the layer keys are only known from the parameters
community_network_keys   = ['n_partnerships', 'mean_degree', 'within_edge_frac', 'single_frac']

# Named groups accepted wherever a community-results selection is expected. 'all' is everything;
# the others are the subsets that are usually wanted on their own.
community_result_presets = dict(
    all      = None,  # filled in by community_result_stems()
    epi      = community_epi_flow_keys + community_epi_rate_keys + ['n_alive'],
    rates    = community_epi_rate_keys + ['n_alive'],
    flows    = community_epi_flow_keys,
    stocks   = community_epi_stock_keys + community_denom_keys,
    network  = community_network_keys + ['n_alive'],
    minimal  = ['n_alive', 'hpv_prevalence', 'cancer_incidence', 'mean_degree'],
)

# What each derived result needs in order to be computable. Anything selected pulls its
# dependencies in with it (see resolve_community_results), so a selection can never produce a
# result that compute_states() cannot fill.
community_result_deps = dict(
    hpv_prevalence    = ['n_infectious', 'n_alive'],
    hpv_incidence     = ['infections', 'n_susceptible'],
    precin_prevalence = ['n_precin', 'n_females_alive'],
    cin_prevalence    = ['n_cin', 'n_females_alive'],
    cancer_incidence  = ['cancers', 'n_cancerous', 'n_females_alive'],
    cancer_mortality  = ['cancer_deaths', 'n_females_alive'],
    mean_degree       = ['n_partnerships', 'n_alive'],
    single_frac       = ['n_alive'],
    within_edge_frac  = ['n_alive'],
)


def community_result_stems(layer_keys=None):
    ''' Every result stem that can be stratified by community, given the sim's layer keys '''
    stems = (community_epi_flow_keys + community_epi_stock_keys + community_denom_keys
             + community_network_keys + community_epi_rate_keys)
    for lkey in (layer_keys or []):
        stems += [f'n_partnerships_{lkey}', f'mean_degree_{lkey}']
    return stems


def resolve_community_results(spec, layer_keys=None, with_deps=True):
    '''
    Turn a community-results selection into the concrete set of result stems to store or export.

    Args:
        spec        : True/'all' for everything, False/None for nothing, a preset name from
                      community_result_presets, or a list of stems. Stems may be given with or
                      without the '_by_community' suffix.
        layer_keys  : the sim's layer keys, which determine the per-layer network stems
        with_deps   : if True, also pull in whatever each selected result needs to be computed

    Returns:
        list of stems, in the canonical order of community_result_stems()
    '''
    available = community_result_stems(layer_keys)
    if spec is None or spec is False:
        return []
    if spec is True:
        wanted = set(available)
    elif isinstance(spec, str):
        if spec not in community_result_presets:
            errormsg = (f'Community results preset "{spec}" not recognised; choices are '
                        f'{sc.strjoin(community_result_presets.keys())}, or pass a list of '
                        f'result names, True for all, or False for none.')
            raise ValueError(errormsg)
        wanted = set(available) if spec == 'all' else set(community_result_presets[spec])
    else:
        wanted = set()
        for key in spec:
            stem = key[:-len('_by_community')] if key.endswith('_by_community') else key
            if stem not in available:
                errormsg = (f'Community result "{key}" not recognised; available results are '
                            f'{sc.strjoin(available)} (with or without the "_by_community" '
                            f'suffix), or use one of the presets '
                            f'{sc.strjoin(community_result_presets.keys())}.')
                raise ValueError(errormsg)
            wanted.add(stem)

    if with_deps: # Iterate to a fixed point, since dependencies can themselves have dependencies
        while True:
            extra = {d for stem in wanted for d in community_result_deps.get(stem, [])} - wanted
            if not extra:
                break
            wanted |= extra
        # Per-layer degrees need the layer partnership counts and the denominator
        for lkey in (layer_keys or []):
            if f'mean_degree_{lkey}' in wanted:
                wanted |= {f'n_partnerships_{lkey}', 'n_alive'}
