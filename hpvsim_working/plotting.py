...
def handle_to_plot(kind, to_plot, n_cols, sim, check_ready=True):
    ...
    time_series_keys = sim.result_keys('total')+sim.result_keys('genotype')+sim.result_keys('sex')+sim.result_keys('type_dist')+sim.result_keys('community') #NEW!! by-community results plot as time series, one line per community
...
def plot_time_series(ax, sim, reskey, resnum, args, colors=None, labels=None, plot_burnin=False):
    ...
    community_keys = sim.result_keys('community') #NEW!!
    ...
    elif reskey in community_keys: #NEW!! one line per community, mirroring the by-genotype case
        community_labels = sim.community_labels()
        c_colors = sc.gridcolors(len(community_labels))
        for community, v_label in enumerate(community_labels):
            color = set_line_options(colors, reskey, resnum, c_colors[community])  # Choose the color
            label = set_line_options(labels, reskey, resnum, res.name)  # Choose the label
            if label:   label += f' - {v_label}'
            else:       label = v_label
            ax.plot(res_t, res.values[community, bi:], label=label, **args.plot, c=color)  # Plot result
