...
class BaseSim(ParsObj):
    ...
    def result_keys(self, which='all'):
        ...
        '''
        keys = []
        subchoices = ['total', 'genotype', 'sex', 'age', 'type_dist', 'community']
        if which in ['total']:
        ...
        elif which in ['community']: #NEW!! only present when pars['community_results'] is on
            keys = [k for k, res in self.results.items() if 'by_community' in k and isinstance(res, Result)]
    ...
    def result_types(self, reskeys):
        ...
        for rkey in reskeys:
            for type_option in ['total', 'genotype', 'sex', 'age', 'type_dist', 'community']: #NEW!! added community
    ...
    def to_df(self, date_index=False, by_community=False):
        '''
        Export results to a pandas dataframe

        Args:
            date_index   (bool): if True, use the date as the index
            by_community : NEW!! which by-community results to include, flattened into one column
                           per community (e.g. hpv_prevalence_by_community_White). False for none
                           (the default), True for every by-community result the sim stored, a
                           preset name from defaults.community_result_presets ('epi', 'rates',
                           'flows', 'stocks', 'network', 'minimal', 'all'), or a list of result
                           names with or without the '_by_community' suffix. Only available if
                           pars['community_results'] is on.

        **Examples**::

            sim.to_df(by_community=True)                             # everything the sim stored
            sim.to_df(by_community='rates')                          # just the prevalences/incidences
            sim.to_df(by_community=['hpv_prevalence', 'mean_degree']) # exactly these two
        '''
        resdict = self.export_results(for_json=False)
        onedim = {k:v for k,v in resdict.items() if v.ndim == 1}
        df = sc.dataframe.from_dict(onedim)
        ...
        #NEW!! Wide-format by-community columns. Every other stratified result (by age, by genotype,
        # by sex) is still dropped above, since only the community stratification is opt-in here
        if by_community is not False and by_community is not None:
            stored = self.result_keys('community')
            if not len(stored):
                errormsg = ("to_df(by_community=...) was requested, but this sim has no "
                            "by-community results. Set pars['community_results'] (which also "
                            "requires network='community') before running.")
                raise ValueError(errormsg)
            if by_community is True:
                keys = stored # Everything the sim actually stored, in results order
            else:
                # Selecting a subset for export only: no dependency expansion, since nothing is
                # being computed here -- the user gets exactly the columns they asked for
                stems = hpd.resolve_community_results(by_community, self.layer_keys(), with_deps=False)
                keys = [f'{stem}_by_community' for stem in stems]
                missing = [k for k in keys if k not in stored]
                if missing:
                    errormsg = (f'Requested by-community results {sc.strjoin(missing)} were not '
                                f'stored by this sim. It stored {sc.strjoin(stored)}; widen '
                                f"pars['community_results'] to include the rest.")
                    raise ValueError(errormsg)
            labels = self.community_labels()
            newcols = {}
            for key in keys:
                vals = resdict[key]
                for i, label in enumerate(labels):
                    newcols[f'{key}_{label}'] = vals[i, :]
            df = df.join(sc.dataframe.from_dict(newcols)) # Joined in one go; inserting the columns one at a time fragments the frame
...
'''
    ...
    def init_contacts(self, reset=False):
        ...
        if reset or not hasattr(self, 'next_eid'): # NEW!! monotonic counter used to assign persistent edge ids
            self.next_eid = 0
    ...
    def add_contacts(self, contacts, lkey=None, beta=None):
        ...
        for lkey, new_layer in new_contacts.items():
            if len(new_layer)>0:
                ...
                # NEW!! Assign persistent edge ids to any new edges that don't already have one.
                # This is the single choke point through which all new contacts pass (initial
                # population creation and every subsequent create_partnerships call), so it is
                # used to give every edge a permanent id that the network delta history can use
                # to track a specific partnership's lifetime, independent of its position in the
                # underlying arrays (which shifts as edges are dissolved and appended over time).
                if 'eid' not in new_layer.keys() or len(new_layer['eid']) != n:
                    start_eid = self.next_eid
                    new_layer['eid'] = np.arange(start_eid, start_eid+n, dtype=hpd.default_int)
                    self.next_eid = start_eid + n
        ...
        return new_contacts # NEW!! return the (now eid-enriched) contacts so callers can capture exactly what was added
...
class Layer(FlexDict):
    ...
    **Examples**::
    ...
    def __init__(self, *args, label=None, **kwargs):
        self.meta = {
            'eid':          hpd.default_int,   # NEW!! Persistent edge id, used to track this partnership across timesteps for network delta history
